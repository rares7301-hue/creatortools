...
function generateIdea() {
  document.getElementById("idea").textContent = pick(ideas);
}

function generateUsername() {
  document.getElementById("username").textContent =
    pick(names) + pick(names) + Math.floor(Math.random() * 100);
}

function generateCaption() {
  document.getElementById("caption").textContent = pick(captions);
}
